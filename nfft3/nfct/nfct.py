...
class NFCT:
    def __init__(self, *args):
        if len(args) == 2:
            ...
            self._plan = _libnfct.jnfct_alloc()
            ...
            _libnfct.jnfct_init(self._plan, self._D, self.__N, self.__M, self.__n, self.__m, self.__f1, self.__f2)
    ...
    def f(self, value):
        ...
        _libnfct.jnfct_set_f.restype = np.ctypeslib.ndpointer(np.float64, ndim=1, shape=(self.M,), flags='C') 
        self._f = _libnfct.jnfct_set_f(self._plan, value)
    ...
    def fhat(self, value):
        ...
        self._fhat = _libnfct.jnfct_set_fhat(self._plan, value)   
    ...
    def trafo_direct(self):
        ...
        _libnfct.jnfct_trafo.restype = np.ctypeslib.ndpointer(np.float64, ndim=1, shape=(self.M,), flags='C') 
        self.f = _libnfct.jnfct_trafo_direct(self._plan)
    ...
    def trafo(self):
        if not self._init_done:
            ...
            raise RuntimeError("fhat has not been set.")
        _libnfct.jnfct_trafo.restype = np.ctypeslib.ndpointer(np.float64, ndim=1, shape=(self.M,), flags='C') 
        self.f = _libnfct.jnfct_trafo(self._plan)
